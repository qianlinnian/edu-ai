...
function onRehydrateStorage() {
  return (state?: SessionState, error?: unknown) => {
...
export const useSessionStore = create<SessionState>()(
  persist(
    (set) => ({
      sessions: [],
      activeId: null,
      setSessions: (sessions: PersistedSession[]) => set({ sessions }),
      setActiveId: (id: number | null) => set({ activeId: id }),
      upsertSession: (session: PersistedSession) =>
      ...
      updateSession: (localId: number, updater: (s: PersistedSession) => PersistedSession) =>
        set((state: SessionState) => {
